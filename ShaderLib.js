...
void main(){
    ...
    if(ldp > 0.)
    {
    	...
    	if(rt > dis)
    	{
    		...
    		float weight = punctualLightIntensityToIrradianceFactor(dis / 3., light.distance, 2.);
...
void main(){
	...
	if(dt < DIST_MAX && dot( n, direction ) < 0.){
		...
		// if(dt < oc * DIST_MAX){
			color = vec4(texture2D(lightBuffer, uv2).xyz * punctualLightIntensityToIrradianceFactor(dt, DIST_MAX, 2.), dt / DIST_MAX) ;
		// }else{
		// 	color = oc;
		// }
	}
...
void main(){
	vec2 uv = gl_FragCoord / res;
	vec4 oc = texture2D(buffer1 , uv);
	vec4 nc = texture2D(buffer2 , uv);

	// gl_FragColor = mix(oc , nc , 1./frame);
	gl_FragColor = vec4(1.);
}
